...
interface IMangaNFT {
    ...
    function mangaChapters(uint256 tokenId) external view returns (
...
contract MonthlyDataUploader is Ownable {
    ...
    event CreatorDataUploaded(uint256 indexed yearMonth, address indexed creator, uint256 monthlyPublished, uint256 totalPublished, uint256 monthlyAcquired, uint256 currentHeld);
    event InvestorDataUploaded(uint256 indexed yearMonth, address indexed investor, uint256 monthlyAcquired, uint256 totalAcquired, uint256 currentHeld);
    event CreatorNFTPublished(address indexed creator, uint256 publishedCount, uint256 totalPublished, uint256 acquiredCount);
    ...
    // 记录创作者发布NFT信息
    function recordCreatorPublish(
        address creator,
        uint256 publishedCount,
        uint256 acquiredCount
    ...
    // 记录投资者获得NFT信息
    function recordInvestorAcquire(
        address investor,
        uint256 acquiredCount
    ) external onlyPlatform {
    ...
    // 查询特定创作者某月的数据
    function getCreatorMonthlyData(address creator, uint256 yearMonth) external view returns (CreatorMonthlyData memory) {
    ...
    // 查询特定投资者某月的数据
    function getInvestorMonthlyData(address investor, uint256 yearMonth) external view returns (InvestorMonthlyData memory) {
    ...
    // 获取投资者的统计信息
    function getInvestorStats(address investor) external view returns (
        uint256 totalAcquired,
        uint256 currentHeld
    ) {
    ...
    // 获取创作者某月的发布和获得数量
    function getCreatorMonthlyStats(address creator, uint256 yearMonth) external view returns (
    ...
    // 获取投资者某月的获得数量
    function getInvestorMonthlyStats(address investor, uint256 yearMonth) external view returns (
        uint256 monthlyAcquired
    ) {
    ...
    // 批量获取多个月的创作者数据
    function getCreatorDataByMonths(uint256[] calldata yearMonths) external view returns (CreatorMonthlyData[][] memory) {
    ...
    // 批量获取多个月的投资者数据
    function getInvestorDataByMonths(uint256[] calldata yearMonths) external view returns (InvestorMonthlyData[][] memory) {
